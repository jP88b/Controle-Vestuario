/* ============================================================
   ATUALIZAÇÃO SEGURA - BI + LANÇAMENTO DE ESTOQUE
   Banco: ControleVestuario

   Use este script quando o banco já existe e você NÃO quer apagar dados.
   Ele adiciona colunas faltantes, recria views/procedures/triggers e preserva tabelas/dados.
   ============================================================ */
...
UPDATE dbo.produtos SET preco_venda = ISNULL(preco_venda, 0);
...
UPDATE m
SET
    preco_custo = ISNULL(m.preco_custo, p.preco_custo),
    preco_venda = ISNULL(m.preco_venda, p.preco_venda)
...
BEGIN
    ...
    BEGIN TRY
        ...
        SET @preco_custo = CASE WHEN @preco_custo IS NULL OR @preco_custo < 0 THEN @precoCustoAtual ELSE @preco_custo END;
        SET @preco_venda = CASE WHEN @preco_venda IS NULL OR @preco_venda < 0 THEN @precoVendaAtual ELSE @preco_venda END;
