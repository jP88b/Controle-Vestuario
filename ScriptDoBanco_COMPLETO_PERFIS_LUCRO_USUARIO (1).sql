...
DROP TRIGGER IF EXISTS trg_categoria_historico;
DROP TRIGGER IF EXISTS trg_fornecedor_historico;
GO
...
-- 1. Atualizar a View dos KPIs do Dashboard
ALTER VIEW vw_dashboard_kpis AS
SELECT 
    COUNT(p.id) AS total_produtos,
    SUM(p.quantidade) AS total_itens_estoque,
    SUM(p.quantidade * p.preco_venda) AS valor_total_estoque,
    SUM(p.quantidade * ISNULL(p.custo, p.preco_custo)) AS custo_total_estoque, -- Ajustado para aceitar custo ou preco_custo
    SUM(p.quantidade * p.preco_venda) - SUM(p.quantidade * ISNULL(p.custo, p.preco_custo)) AS lucro_potencial_estoque,
    COUNT(CASE WHEN p.quantidade <= 5 THEN 1 END) AS total_estoque_critico
FROM produtos p;
GO

-- 2. Atualizar a View de Estoque Crítico
ALTER VIEW vw_estoque_critico AS
SELECT 
    p.id AS produto_id,
    p.nome AS produto,
    ...
    ISNULL(p.custo, p.preco_custo) AS custo, -- Garante que o Spring vai ler como "custo"
...
CREATE OR ALTER TRIGGER trg_categoria_historico
ON categorias
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
...
CREATE OR ALTER TRIGGER trg_fornecedor_historico
ON fornecedores
AFTER INSERT, UPDATE, DELETE
AS
BEGIN
...
/* ===== Ajustes finais de perfis, lucro e usuário do sistema ===== */
USE ControleVestuario;
GO
